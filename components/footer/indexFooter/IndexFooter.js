...
const IndexFooter = () => {
  return <div className="container">
      <div className="imgCtn">
          <Image src={resource} layout="fill" objectFit="cover" objectPosition="bottom center" placeholder="blur" />
      </div>
      <span>
      ...
      <div className="socials">
        {icons.map(icon => (
            <a className="social_link" key={icon.id} href={icon.link} target="_blank" rel="noopener noreferrer">
                <Icon icon={icon.iconName} height={icon.iconHeight} color={icon.color}/>
                <div>
                    <h6>{icon.title}</h6>
                    <p>{icon.desc}</p>
                </div>
    ...
    <style jsx>{`
            ...
            .imgCtn::after{
                ...
                position: absolute;
                inset: 0;
                background: linear-gradient(0deg, rgba(24,22,22,1) 0%, rgba(0,0,0,0) 80%);
            }
            h1{
                letter-spacing: 0;
            ...
            span{
                ...
                padding: var(--margin-one) calc(var(--margin-one) + var(--margin-two)) var(--margin-one) var(--margin-one);
            }
            .socials{
